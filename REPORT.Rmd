...
The data set contains `r nrow(utxo_count)` entries.

As we can see the number of rows of `utxo_count` is different since it has more than 1 entry per day, also `fear_and_greed_index` seems to miss one data point. We will see fix that in the next sections.
...
p7 <- candles_enhanced_cleaned %>%
  ...
  labs(
    title = "BTC-USD Candlestick Chart (Last 100 Candles)",
    x = "Time",
    y = "Price"
  ) +
  scale_y_continuous(labels = scales::comma)
...
# For more readability we are only plotting the last 100 candles
p8 <- candles_enhanced_cleaned %>%
  tail(100) %>%
  ggplot(aes(x = time, y = volume)) +
  geom_segment(aes(xend = time, yend = 0, color = volume)) +
  geom_smooth(method = "loess", se = FALSE) +
  labs(title = "BTC-USD Candlestick Chart (Last 100 candles)", y = "Volume", x = "") +
  theme_tq() +
  theme(legend.position = "none")
